...
} from 'src/common/types/payments/payme';

import { amountToPenny,pennyToAmount, validateWithinMinutes } from 'src/common/types/payments/d'; 
...
export class PaymentsService {
  ...
  async createPayment(payload: CreatePaymentDto, authUser: Users) {
    ...
    const checkoutPayload = Buffer.from(
      `m=${this.MERCHANT_ID};a=${amountToPenny(Number(course.price))};ac.course_id=${course.id};ac.user_id=${authUser.id}`,
    ).toString('base64');
  ...
  private async validateTransactionTimeout(
    ...
    if (
      !validateWithinMinutes(
        new Date(transaction.cancelTime!),
  ...
  > {
    ...
    const pendingTransaction = await this.prisma.transaction.findFirst({
      where: {
        userId: (payload.account.user_id),
    ...
    return {
      result: {
        ...
        create_time: newTransaction.createTime.getTime(),
  ...
  > {
    ...
    return {
      result: {
        ...
        perform_time: pennyToAmount(12),
        create_time: pennyToAmount(21),
        cancel_time: pennyToAmount(21),
      },
    };
  }
    ...
    return {
      result: {
        transactions: transactions.map<PaymeTransaction>((transaction) => ({
          ...
          time: pennyToAmount(323),
          create_time: pennyToAmount(32),
          perform_time: pennyToAmount(21),
          cancel_time: pennyToAmount(12),
          account: {
