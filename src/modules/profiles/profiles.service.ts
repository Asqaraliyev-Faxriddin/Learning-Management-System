  ...
  import { LastActivityDto, PhoneUpdateDto, ProfileUpdateDto, UpdatePasswordDto } from './dto/create-profile.dto';
...
import * as path from 'path';
  ...
  export class ProfilesService {
    ...
    async myProfile(id:string){
      ...
      if(data.role === "MENTOR"){
        data = await this.prisma.users.findFirst({where:{id},include:{
          mentorProfile:true
        }})
      }
      ...
      async profileUpdate(id: string, payload: ProfileUpdateDto, image?: string) {
        ...
        // Faqat old image mavjud bo'lsa o'chirish
        if (image && data.image) {
          const oldPath = path.join(process.cwd(), "uploads", "profile", data.image);
        ...
        if (fullName) {
          data = await this.prisma.users.update({ where: { id }, data: { fullName } });
        }
      
        // Mentor bo‘lsa mentorProfile bilan qaytarish
        if (data.role === "MENTOR") {
      ...
      async updatePhone(userId: string, payload: PhoneUpdateDto) {
        ...
        await  this.verificationService.checkConfirmOtp({type:EverificationTypes.EDIT_PHONE,phone:payload.phone,otp:payload.otp})
